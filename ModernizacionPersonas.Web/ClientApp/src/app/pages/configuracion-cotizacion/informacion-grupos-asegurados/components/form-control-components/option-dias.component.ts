...
@Component({
  selector: 'app-option-dias',
  template: `<div class="number-container">
            <input #input type="text" [disabled]="disabled" (change)="onChange($event)"
            value="{{ value }}" [readonly]="readonly" /></div>`,
  styleUrls: ['./styles.scss'],
})
export class OptionDiasComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.model = this.option.numeroDiasOption;
    this.amparo = this.element.amparo;
    this.maxValue = this.element.amparo.modalidad.valores[1].tope;
    ...
    this.value = this.rawValue > 0 ? this.model.value : '';
  ...
  onChange(e) {
    ...
    if (this.rawValue > this.maxValue) {
      ...
      this.notificationService.showAlert(`NO se puede superar el valor máximo de ${this.maxValue} días`);
    } else {
  ...
  private emitValueChange() {
    ...
    this.valoresAseguradosService.emit(ValoresAseguradosEvents.DIAS_VALUE_CHANGE, args);
  }
