...
{
    internal class MSCotizacionDataProcessor : CotizacionDataProcessorBase, IResumenDataProcessor2
    {
        ...
        public MSCotizacionDataProcessor(Entities.CotizacionDataProcessorArgs args)
        ...
        {
            // var codigoTipoTasa = informacionNegocio.CodigoTipoTasa1 == 5 ? informacionNegocio.CodigoTipoTasa2 : informacionNegocio.CodigoTipoTasa1;
            ...
            {
                this.asegurados = await this.aseguradosReader.LeerAseguradosAsync(grupo.CodigoGrupoAsegurado);
            }
            ...
            foreach (var amparo in grupo.AmparosGrupo)
            {
                ...
                var tasaRiesgo = tasaAmparo == null ? 0 : tasaAmparo.Tasa;
                
                var opciones = amparo.OpcionesValores.Where(x => x.ValorAsegurado > 0 || x.NumeroSalarios > 0 || x.Prima > 0);
                foreach (var opcion in opciones)
                {

                    var valorAseguradoAmparo = this.CalcularValorAseguradoAmparoOpcion(grupo, amparo, opcion);
                    ...
                    var primaAmparo = esAsistencia ? opcion.Prima : this.CalcularPrimaOpcion(tasaComercialAplicar, valorAseguradoAmparo);
                    var primaAmparoSinAplicar = esAsistencia ? opcion.Prima : this.CalcularPrimaOpcion(tasaComercial, valorAseguradoAmparo);
                    if (esAmparoBasicoNoAdicional && informacionNegocio.ConListaAsegurados)
                    {
                        ...
                        primaAmparo = esTipoSumaAseguradaSalario
                            ? this.codigoTipoTasa == 1 ? this.CalcularPrimaOpcion(tasaComercialAplicar, valorAseguradoAmparo) 
                            : this.CalcularPrimaOpcionPorSalario(factorG, tasasResult.TasasAsegurados, opcion.NumeroSalarios, codigoTipoTasa)
                            : grupo.CodigoTipoSuma == 6 ? esAsistencia ? opcion.Prima : this.CalcularPrimaOpcion(tasaComercialAplicar, valorAseguradoAmparo) : this.CalcularPrimaOpcionPorAsegurados(factorG, tasasResult.TasasAsegurados);
                        primaAmparoSinAplicar = esTipoSumaAseguradaSalario
                            ? this.codigoTipoTasa == 1 ? this.CalcularPrimaOpcion(tasaComercial, valorAseguradoAmparo)
                            : this.CalcularPrimaOpcionPorSalario(factorG, tasasResult.TasasAsegurados, opcion.NumeroSalarios, codigoTipoTasa)
                            : grupo.CodigoTipoSuma == 6 ? esAsistencia ? opcion.Prima : this.CalcularPrimaOpcion(tasaComercial, valorAseguradoAmparo) : this.CalcularPrimaOpcionPorAsegurados(factorG, tasasResult.TasasAsegurados);

                        if (codigoTipoTasa == 4)
                        {
                            primaAmparo = tasaComercialAplicar * valorAseguradoAmparo / 1000;
                            primaAmparoSinAplicar = tasaComercial * valorAseguradoAmparo / 1000;
                        }
                    }
                    else if (!esAmparoBasicoNoAdicional && informacionNegocio.ConListaAsegurados)
                    {
                        ...
                        tasaComercialAplicar = tasaComercial - (tasaComercial * tasasOpcion.Descuento / 100) + (tasaComercial * tasasOpcion.Recargo / 100);
                        primaAmparo = esAsistencia ? opcion.Prima : this.CalcularPrimaOpcion(tasaComercialAplicar, valorAseguradoAmparo);
                        primaAmparoSinAplicar = esAsistencia ? opcion.Prima : this.CalcularPrimaOpcion(tasaComercial, valorAseguradoAmparo);
                    ...
                    opcion.Prima = esAsistencia ? valorAseguradoAmparo : primaAmparo;
                    opcion.PrimaSinAplicar = esAsistencia ? valorAseguradoAmparo : primaAmparoSinAplicar;
                    opcion.ValorAsegurado = valorAseguradoAmparo;
                }
            }
        ...
        private decimal CalcularValorAseguradoAmparoOpcion(GrupoAsegurado grupo, AmparoGrupoAsegurado amparo, OpcionValorAsegurado opcion)
        {
            ...
            var numeroSalarios = amparoBasicoNoAdicional.OpcionesValores.FirstOrDefault().NumeroSalarios;
            var esAmparoBasicoNoAdicional = amparo.CodigoAmparo == amparoBasicoNoAdicional.CodigoAmparo;
            var opcionBase = amparoBasicoNoAdicional.OpcionesValores[opcion.IndiceOpcion - 1];
            if (esAmparoBasicoNoAdicional)
            {
                
                result = grupo.ValorAsegurado;
                // Si es Tipo Tasa Edad por Cada Asegurado
                if (codigoTipoTasa == 2)
                {
                    result = this.asegurados.Sum(x => x.ValorAsegurado * opcion.NumeroSalarios);
                }
            }
            else
            {
                result = opcion.PorcentajeCobertura > 0
                    ? opcionBase.ValorAsegurado * opcion.PorcentajeCobertura / 100
                    : opcion.ValorDiario > 0 ? opcion.ValorAsegurado * grupo.NumeroAsegurados:
                    // Corrección 30/01/2022 Salarios no afectaban vlaro asegurado amparos adicionales
                    (grupo.ValorAsegurado * opcion.NumeroSalarios) / numeroSalarios;
            }
        ...
        public decimal CalcularValorAseguradoTotal(GrupoAsegurado grupo, OpcionValorAsegurado opcion, bool conListaAsegurados)
        {
            ...
            var result = codigoTipoTasa==5 || codigoTipoTasa == 1 || codigoTipoTasa == 4 ? grupo.ValorAsegurado :  grupo.ValorAsegurado * opcion.NumeroSalarios;
        ...
        private async Task<ValorAseguradoOpcionResumen> ProcessOpcionAmparoAsync(InformacionNegocio informacionNegocio, GrupoAsegurado grupo, AmparoGrupoAsegurado amparo, OpcionValorAsegurado o)
        {
            ...
            var tasaComercialAnual = this.CalcularTasaAnual(primaTotalAnualNoAfecta - valorAsistencia, valorAseguradoTotal);
            ...
            tasaOpcion.PrimaTotal = Math.Round(primaTotalAnual, 0);
            ...
            var opcion = new ValorAseguradoOpcionResumen
            {
                ...
                PrimaAnualIndividual = Math.Round(primaAnual,0),
                PrimaAnualTotal = Math.Round(primaTotalAnual,0),
        ...
        public async Task<PrimasGrupoAsegurado> CalcularPrimasGrupoAseguradoFichaTecnicaAsync(InformacionNegocio informacionNegocio, GrupoAsegurado grupo)
        {
            ...
            for (int i = 0; i < opcionesCount; i++)
            {
                ...
                // calcula primas por grupo
                // TODO uncomments after fix UpdateTasaOPcion metod, el metodo UpdateTasaOpcion deja en 0 el valor del a prima anual
                // var primaAnual = tasaOpcion.SumatoriaTasa;
                var primaAnual = sumPrimaAmparoOpcion;
                var primaTotalAnual = this.CalcularPrimaAnualTotal(primaAnual, valorAsistencia);
                if (this.tieneSiniestralidad)
                {
                ...
                tasaOpcion.PrimaIndividual = Math.Round(primaTotalAnual, 0);
                tasaOpcion.PrimaTotal = Math.Round(primaTotalAnual, 0);
