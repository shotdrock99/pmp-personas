...
{
    internal class SFMSCotizacionDataProcessor : CotizacionDataProcessorBase, IResumenDataProcessor2
    {
        ...
        public SFMSCotizacionDataProcessor(Entities.CotizacionDataProcessorArgs args)
        ...
        public async Task<GrupoAseguradoResumen> BuildGrupoAseguradoResumen(InformacionNegocio informacionNegocio, GrupoAsegurado grupo)
        {
            ...
            foreach (var amparo in grupo.AmparosGrupo)
            {
                ...
                var tasaRiesgo = tasaAmparo.Tasa;
                ...
                {
                    var valorAseguradoOpcion = this.CalcularValorAseguradoAmparoOpcion(grupo, amparo, opcion);
                    ...
                    var primaAmparo = esAsistencia ? opcion.Prima : this.CalcularPrimaOpcion(tasaComercial, valorAseguradoOpcion);
                    ...
                    if (esAmparoBasicoNoAdicional && informacionNegocio.ConListaAsegurados)
                    {
                        ...
                        primaAmparo = esTipoSumaAseguradaSalario
                             ? amparo.AmparoInfo.SiNoRequiereEdades == true ? this.CalcularPrimaOpcion(tasaComercialAplicar, valorAseguradoOpcion) 
                        ...
                        primaAmparoSinAplicar = esTipoSumaAseguradaSalario
                            ? amparo.AmparoInfo.SiNoRequiereEdades == true ? this.CalcularPrimaOpcion(tasaComercialAplicar, valorAseguradoOpcion)
                            : this.CalcularPrimaOpcionPorSalario(factorG, tasasResult.TasasAsegurados, opcion.NumeroSalarios, codigoTipoTasa)
                            : grupo.CodigoTipoSuma == 1 ? esAsistencia ? opcion.Prima : this.CalcularPrimaOpcion(tasaComercial, valorAseguradoOpcion) : amparo.AmparoInfo.SiNoRequiereEdades == true ? this.CalcularPrimaOpcion(tasaComercial, valorAseguradoOpcion) : this.CalcularPrimaOpcionPorAsegurados(factorG, tasasResult.TasasAsegurados);
                        if (codigoTipoTasa == 4)
                        {
        ...
        private decimal CalcularValorAseguradoAmparoOpcion(GrupoAsegurado grupo, AmparoGrupoAsegurado amparo, OpcionValorAsegurado opcion)
        {
            ...
            if (opcionBase.NumeroSalarios > 0)
            {
                return this.CalcularPorMultiploSueldos(grupo, amparo, opcion);
            }

            return this.CalcularPorSumaFija(grupo, amparo, opcion);
        }

        private decimal CalcularPorMultiploSueldos(GrupoAsegurado grupo, AmparoGrupoAsegurado amparo, OpcionValorAsegurado opcion)
        {
            var amparoBasicoNoAdicional = grupo.AmparosGrupo.Where(x => x.AmparoInfo.SiNoBasico && !x.AmparoInfo.SiNoAdicional).FirstOrDefault();
            var esAmparoBasicoNoAdicional = amparo.AmparoInfo.SiNoBasico && !amparo.AmparoInfo.SiNoAdicional;
            var opcionBase = amparoBasicoNoAdicional.OpcionesValores[opcion.IndiceOpcion - 1];
            var result = grupo.ValorAsegurado * opcion.NumeroSalarios;
            ...
            else
            {
                ...
                {
                    //result = opcion.ValorAsegurado * opcion.NumeroSalarios; // TODO: MS Tasa es igual a Valor Nómina * N° de Sueldos 
                    result = opcion.ValorAsegurado * grupo.NumeroAsegurados;
                    if (opcion.NumeroSalarios > 0)
                    {
                        result = opcion.NumeroSalarios * grupo.ValorAsegurado;
                    }
                }
            }
        ...
        private decimal CalcularPorSumaFija(GrupoAsegurado grupo, AmparoGrupoAsegurado amparo, OpcionValorAsegurado opcion)
        {
            var amparoBasicoNoAdicional = grupo.AmparosGrupo.Where(x => x.AmparoInfo.SiNoBasico && !x.AmparoInfo.SiNoAdicional).FirstOrDefault();
            var opcionBase = amparoBasicoNoAdicional.OpcionesValores[opcion.IndiceOpcion - 1];
            // Cambio revisión César FT, multiplciar valor asegurado por N° Asegurados.
            var result = opcion.ValorAseguradoDias > 0 ? opcion.ValorAseguradoDias == opcion.ValorAsegurado ? opcion.ValorAsegurado * grupo.NumeroAsegurados : opcion.ValorAsegurado : opcion.ValorAsegurado;
            if (amparo.AmparoInfo.SiNoPorcentajeBasico)
            {
                result = opcionBase.ValorAsegurado * (opcion.PorcentajeCobertura / 100);
            }
        ...
        private decimal CalcularPrimaAnualTotal(decimal sumPrimaAmparoOpcion)
        {
            // TODO calcular prima
            return sumPrimaAmparoOpcion;
        }
        ...
        public decimal CalcularValorAseguradoTotal(GrupoAsegurado grupo, OpcionValorAsegurado opcion, bool conListaAsegurados)
        {
            var amparoBasicoNoAdicional = grupo.AmparosGrupo.Where(x => x.AmparoInfo.SiNoBasico && !x.AmparoInfo.SiNoAdicional).FirstOrDefault();
            var opcionBase = amparoBasicoNoAdicional.OpcionesValores[opcion.IndiceOpcion - 1];
            var  result = opcionBase.ValorAsegurado > 0 ? amparoBasicoNoAdicional.OpcionesValores[opcion.IndiceOpcion - 1].ValorAsegurado : grupo.ValorAsegurado * opcionBase.NumeroSalarios;
        ...
        public async Task<PrimasGrupoAsegurado> CalcularPrimasGrupoAseguradoFichaTecnicaAsync(InformacionNegocio informacionNegocio, GrupoAsegurado grupo)
        {
            ...
            for (int i = 0; i < opcionesCount; i++)
            {
                ...
                var primaAnual = tasaOpcion.SumatoriaTasa + valorAsistencia;
                ...
                tasaOpcion.PrimaTotal = Math.Round(primaAnual, 6);
                tasaOpcion.PrimaIndividual = Math.Round(primaAnual, 6);
