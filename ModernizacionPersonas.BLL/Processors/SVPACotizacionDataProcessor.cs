...
{
    internal class SVPACotizacionDataProcessor : CotizacionDataProcessorBase, IResumenDataProcessor2
    {
        ...
        public SVPACotizacionDataProcessor(Entities.CotizacionDataProcessorArgs args)
        ...
        public async Task<GrupoAseguradoResumen> BuildGrupoAseguradoResumen(InformacionNegocio informacionNegocio, GrupoAsegurado grupo)
        {
            ...
            foreach (var amparo in grupo.AmparosGrupo)
            {
                ...
                foreach (var opcion in opciones)
                {
                    ...
                    if (esAmparoBasicoNoAdicional && informacionNegocio.ConListaAsegurados)
                    {
                       ...
                       // var edadPromedio = Math.Round(this.asegurados.Average(x => x.Edad));
                        var edadPromedio = grupo.EdadPromedioAsegurados;
                        ...
                        primaAmparo = esTipoSumaAseguradaSalario
                           ...
                           : grupo.CodigoTipoSuma == 6 ? esAsistencia ? opcion.Prima : this.CalcularPrimaOpcion(tasaComercialAplicar, valorAseguradoOpcion) : amparo.AmparoInfo.SiNoRequiereEdades == true ? this.CalcularPrimaOpcion(tasaComercialAplicar, valorAseguradoOpcion) : this.CalcularPrimaOpcionPorAsegurados(factorG, tasasResult.TasasAsegurados);
                        primaAmparoSinAplicar = esTipoSumaAseguradaSalario
                            ? this.CalcularPrimaOpcionPorSalario(factorG, tasasResult.TasasAsegurados, opcion.NumeroSalarios, codigoTipoTasa)
                            : grupo.CodigoTipoSuma == 6 ? esAsistencia ? opcion.Prima : this.CalcularPrimaOpcion(tasaComercial, valorAseguradoOpcion) : codigoTipoTasa == 2 ? amparo.AmparoInfo.SiNoRequiereEdades == true ? this.CalcularPrimaOpcion(tasaComercial, valorAseguradoOpcion) : this.CalcularPrimaOpcionPorAsegurados(factorG, tasasResult.TasasAsegurados) : this.CalcularPrimaOpcion(tasaComercial, valorAseguradoOpcion);
                        if (codigoTipoTasa == 4)
                        {
                            primaAmparo = tasaComercial * valorAseguradoOpcion / 1000;
            ...
            if (tieneSiniestralidad)
            {
                ...
                var primaIndividualAnio = primasAmparos.Where(x => x.IndexOption == 1).Sum(x => x.Value);
                var primasxAmparo = primasAmparos.Where(x => x.IndexOption == 1).Select(x => x);
        ...
        public decimal CalcularValorAseguradoAmparoOpcion(GrupoAsegurado grupo, AmparoGrupoAsegurado amparo, OpcionValorAsegurado opcion, int numeroAsegurados)
        {
            var esAsistencia = amparo.AmparoInfo.CodigoGrupoAmparo == 3;
            if (esAsistencia)
            {
        ...
        public async Task<PrimasGrupoAsegurado> CalcularPrimasGrupoAseguradoFichaTecnicaAsync(InformacionNegocio informacionNegocio, GrupoAsegurado grupo)
        {
            ...
            for (int i = 0; i < opcionesCount; i++)
            {
                ...
                // calcula primas por gruposs
                // 20-01-2022 Se modifica la formula de PrimaAnual para incluir el valor de la asistencia * N° de asegurados
                // 12-04-2022 Se modific+o la obtención de assitencia para evitar multiplicar por el número de asegurados
